...
String svg = "M247,27.5c0,78.294-58,79-58,0";//M218.5,86L218.5,28M124.067,18c16.555,0,6.816,13.976,0,13.976c-7.79,0-5.328,11.305,33.337,0C196.07,20.67,164.257,84.029,125.184,86M96.088,28c5.883,0,8.089,37.707,8.089,58.486M32.509,32.531c11.073-12.947,33.264-7.632,40.795-0.427c7.53,7.205,11.297,38.685,0,49.932c-11.298,11.248-32.635,10.092-40.795,0C24.351,71.944,24.351,42.07,32.509,32.531";
...
void drawSection(String type, String data){
  ...
  for (int i=0; i<3; i++){
    ...
    if (nextY > 0){
      ...
      y[i] = atof(nums);
      data = data.substring(nextY);
    }else if (data.length() > 0){
      data.substring(0,nextX).toCharArray(nums, 5);
      x[i] = atof(nums);
      data.substring(nextX).toCharArray(nums, 5);
      y[i] = atof(nums);
      data = "";
    }else{
      x[i] = -1;
